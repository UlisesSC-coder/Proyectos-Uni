...
<head>
    <meta charset="UTF-8">
    <title>Catastro - Eliminar Propietario</title>
    <link rel="stylesheet" href="../css/estilos.css?v=<?php echo time(); ?>">
    <script src="../javascript/confirmar_baja_propietario.js?v=<?php echo time(); ?>" defer></script>
</head>
<body class="dashboard-page">
    ...
    <div class="tabla-container-canva container-bajas">
        <h2>Baja de Registro de Propietarios</h2>
        <p class="texto-advertencia">⚠️ Nota: Para eliminar un propietario, asegúrate de que no tenga predios vigentes enlazados debido a la integridad de la base de datos.</p>
        <br>
            ...
            <table class="tabla-resumen tabla-bajas">
                ...
                <tbody>
                    <?php foreach ($propietarios as $p): ?>
                        <tr>
                            <td class="td-id-baja">#<?php echo $p['id_propietario']; ?></td>
                            <td class="td-nombre-baja">
                                <?php echo htmlspecialchars($p['nombre'] . ' ' . $p['apellido_paterno'] . ' ' . $p['apellido_materno']); ?>
                            </td>
                            <td>
                                <span class="badge-mono"><?php echo htmlspecialchars($p['rfc']); ?></span>
                            </td>
                            <td>
                                <span class="badge-mono"><?php echo htmlspecialchars($p['curp']); ?></span>
                            </td>
                            <td class="td-ellipsis domicilio-width" title="<?php echo htmlspecialchars($p['domicilio']); ?>">
                                <?php echo htmlspecialchars($p['domicilio']); ?>
                            </td>
                            <td class="td-nowrap"><?php echo htmlspecialchars($p['telefono']); ?></td>
                            <td class="td-ellipsis correo-width" title="<?php echo htmlspecialchars($p['correo_electronico']); ?>">
                                <?php echo htmlspecialchars($p['correo_electronico']); ?>
                            </td>
                            <td class="td-nowrap"><?php echo htmlspecialchars($p['fecha_nacimiento']); ?></td>
                            <td class="td-accion">
                                <button type="button" class="btn-eliminar-baja" data-id="<?php echo $p['id_propietario']; ?>">Eliminar</button>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
